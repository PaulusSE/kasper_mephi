...
//	@Router			/administrator/students/report/download/{token} [post]
func (h *AdministratorHandler) GetPresentation(ctx *gin.Context) {
	...
	presentationData, err := h.presentation.GetPresentation(ctx, user.KasperID)
	if err != nil {
		ctx.AbortWithStatusJSON(models.MapErrorToCode(err), gin.H{"error": err.Error()})
		return
	}

	load, err := h.presentation.GetStudentLoad(ctx, user.KasperID, int32(requestData.Semester))
	if err != nil {
		ctx.AbortWithStatusJSON(models.MapErrorToCode(err), gin.H{"error": err.Error()})
		return
	}
