...
# Все необходимые данные из JSON (всё, что может меняться)
university_full_name = input_data['university_full_name']
institute_and_department = input_data['institute_and_department']
education_direction = input_data['education_direction']
education_direction_code = input_data['education_direction_code']
education_profile = input_data['education_profile']
education_profile_code = input_data['education_profile_code']
...
# Пути к логотипам (тоже могут быть переданы в JSON!)
logo_mephi = input_data.get('logo_mephi', 'MEPhI_Logo2014_en.png')
logo_kafedra = input_data.get('logo_kafedra', 'kaf22.png')
...
class SlideGenerator:
    ...
    def add_image_to_slide(self, slide, image_path, left, top, width, height):
            try:
                ...
                print(f"Error adding image {image_path}: {e}", file=sys.stderr)
...
def build_table_disser_work_data(topic, progress_percents, progress_descriptions):
    ...
    for i in range(len(semesters)):
        ...
        table_data.append([("", 1, 0), (semesters[i], 1, 0), (percent, 1, 0), (description, 1, 0)])
    return table_data
...
def build_table_publications_data(publications):
    ...
    column_widths = [
        Inches(0.5), Inches(2.25), Inches(0.8),
        Inches(0.8), Inches(2.5), Inches(0.7), Inches(1.2)
    ]
...
def build_table_publ_all(data):
    ...
    table_data = [[(header, 1, 1) for header in headers]]
    for i, row in enumerate(data):
        row_data = [(rows[i], 1, 1)]
        ...
        for j in range(6):
            ...
            if value != '':
                total_for_semester += int(value)
            row_data.append((str(value), 1, 1))
        row_data.append((str(total_for_semester) if total_for_semester != 0 else '', 1, 1))
...
progress_percents = input_data.get('progress_percents', [])
if len(progress_percents) < 8:
    progress_percents += [0] * (8 - len(progress_percents))
progress_descriptions = input_data.get('progress_descriptions') or []
publications = input_data.get('publications') or []
all_publications = input_data.get('all_publications') or []
pedagogical_data = input_data.get('pedagogical_data') or []
report_other_achievments = input_data.get('report_other_achievments') or ""
pedagogical_data_all = input_data.get('pedagogical_data_all') or []
next_semester_plan = input_data.get('next_semester_plan') or []


slides_data = [
    [
        {'type': 'text', 'content': university_full_name, 'left': Pt(50), 'top': Pt(50), 'width': Pt(620), 'height': Pt(60), 'font_size': 12, 'bold': False, 'alignment': PP_ALIGN.CENTER},
        {'type': 'text', 'content': institute_and_department, 'left': Pt(50), 'top': Pt(130), 'width': Pt(620), 'height': Pt(40), 'font_size': 12, 'bold': True, 'alignment': PP_ALIGN.CENTER},
        {'type': 'text', 'content': f"НАПРАВЛЕНИЕ ПОДГОТОВКИ {education_direction_code} {education_direction}", 'left': Pt(50), 'top': Pt(190), 'width': Pt(620), 'height': Pt(40), 'font_size': 12, 'bold': True, 'alignment': PP_ALIGN.CENTER},
        {'type': 'text', 'content': f"ПРОФИЛЬ ПОДГОТОВКИ {education_profile_code} {education_profile}", 'left': Pt(50), 'top': Pt(250), 'width': Pt(620), 'height': Pt(40), 'font_size': 12, 'bold': True, 'alignment': PP_ALIGN.CENTER},
        {'type': 'text', 'content': report_title, 'left': Pt(50), 'top': Pt(310), 'width': Pt(620), 'height': Pt(60), 'font_size': 30, 'bold': False, 'alignment': PP_ALIGN.CENTER},
        ...
        {'type': 'text', 'content': f"{city}, {year}", 'left': Pt(50), 'top': Pt(450), 'width': Pt(620), 'height': Pt(30), 'font_size': 16, 'bold': False, 'alignment': PP_ALIGN.CENTER},
        ...
        {'type': 'image', 'image_path': logo_mephi, 'left': Inches(0), 'top': Inches(6.7), 'width': Inches(0.8), 'height': Inches(0.8)},
        {'type': 'image', 'image_path': logo_kafedra, 'left': Inches(8.5), 'top': Inches(6.9), 'width': Inches(1.5), 'height': Inches(0.6)}
    ],
    [
        ...
        {'type': 'table', 'data': [
            [("Общие сведения на текущий момент", 1, 4)],
